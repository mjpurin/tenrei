<table>
  <tbody>
    <tr>
      <td width="220" nowrap="nowrap">&nbsp;</td>
      <td rowspan="2" width="4">&nbsp;</td>
      <td width="164">&nbsp;</td>
      <td rowspan="2" width="1">&nbsp;</td>
      <td rowspan="2" width="1">&nbsp;</td>
      <td width="336" rowspan="4" align="right" valign="top">
        <!--右カラム-->
        <table>
        <tbody>
        <tr valign="bottom">
          <td height="151" style="padding-top:16px;"><div align="center"><img src="img/jikiso.jpg" width="400" height="238"></div>
          </td>
        </tr>
        <!--写真-->
        <tr>
          <td style="padding:10px 0 10px 50px;">《直葬施行例》<span style="font-size:12px">　※写真＝ホールリニューアル前</span></td>
        </tr>
        <tr>
          <td align="center">&nbsp;</td>
        </tr>
        <!--左真ん中-->
        <tr>
          <td align="center" valign="top" style="background:#eee;padding:12px 5px;">
          <table width="280">
          <tbody>
            <tr>
              <td align="center" valign="top">
                <table width="245" border="0">
                  <tbody >
                    <tr align="left">
                      <td colspan="3" nowrap="nowrap" style="padding-bottom:2px;border-bottom:1px dotted #000;">その他の費用<span style="font-size:14px;">（かわさき北部斎苑利用の時）</span>50人の場合</td>
                    </tr>
                    <tr >
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:10px;">返礼品</td>
                      <td width="12" nowrap="nowrap" style="padding-top:10px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:10px;"><div align="right">-</div></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">飲 食<span style="margin-left:5px;font-size:14px">初七日</span></td>
                      <td width="12" nowrap="nowrap" style="padding-top:5px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">-</div></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">追加ドライアイス</td>
                      <td width="12" nowrap="nowrap" style="padding-top:5px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">-</div></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">合　　計</td>
                      <td width="12" nowrap="nowrap"></td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">-</div></td>
                    </tr>
                    <tr align="left">
                        <td colspan="3" nowrap="nowrap" style="text-align:right;"><span style="font-size:12px">*その他特注品は含みません</span></td>
                    </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
          </td>
          </tr>
          <!--空白-->
          <tr>
              <td align="center">&nbsp;</td>
          </tr>
          <!--右下-->
          <tr>
          <td align="center" valign="top" style="background:#eee;padding:12px 5px;">
          <table width="280">
          <tbody>
            <tr>
              <td align="center" valign="top">
                <table width="245" border="0">
                  <tbody >
                    <tr align="left">
                      <td colspan="3" nowrap="nowrap" style="padding-right:113px;padding-bottom:2px;border-bottom:1px dotted #000;">実費費用<span style="font-size:14px;">（かわさき北部斎苑利用の時）</span></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">式場費（斎苑第2）100人用</td>
                      <td width="12" nowrap="nowrap" style="padding-top:5px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">-</div></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">火葬料</td>
                      <td width="12" nowrap="nowrap" style="padding-top:5px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">6,750</div></td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">休憩室(50人用)</td>
                      <td width="12" nowrap="nowrap" style="padding-top:5px;">＝</td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">6,000</div></td>
                    </tr>
                    <tr>
                      <td colspan="3" nowrap="nowrap" style="padding-top:5px;">宗教者へお礼（宗旨宗派により金額が異なる）</td>
                    </tr>
                    <tr>
                      <td width="189" align="left" nowrap="nowrap" style="padding-top:5px;">合　　計</td>
                      <td width="12" nowrap="nowrap"></td>
                      <td width="58" nowrap="nowrap" style="padding-top:5px;"><div align="right">12,750</div></td>
                    </tr>
                    
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
          </td>
          </tr>
        </tbody>
      </table>
      <div align="center"></div>
      </td>
    </tr>
    
    <!--左-->
    <tr>
      <td width="220">
        <table width="168" border="1">
          <tbody>
            <tr>
              <td style="border:1px solid #000;text-align:center;font-size:14px;padding:5px;">セット名称（形式）</td>
            </tr>

            <tr>
              <td style="border:1px solid #000;text-align:center;font-size:18px;padding:5px;font-weight:bold;">直葬(火葬のみ)</td>
            </tr>
              </tbody>
            </table>
          </td>
            <td>
                <table>
                  <tbody>
                  <tr>
                    <td style="border:1px solid #000;text-align:center;font-size:14px;padding:5px 20px;">提供価格(税込)</td>
                  </tr>
                  <tr>
                    <td style="border:1px solid #000;text-align:center;font-size:18px;padding:5px 20px;font-weight:bold;">275,000円</td>
                  </tr>
                </tbody></table>
              <div align="center"></div></td>
          </tr>
          <tr>
            <td colspan="4" style="padding:20px 0 5px;font-size:16px;">《内　容》　（令和2年12月現在）</td>
            <td width="1" height="12">&nbsp;</td>
          </tr>
          <tr>
            <td colspan="4" valign="top">
              <!--項目一覧テーブル-->
              <table id="priceMainTable">
                <tbody>
                  <tr>
                  <th>項目</th>
                  <th>内容</th>
                </tr>
                <tr>
                  <td>祭壇</td>
                  <td>無し（お別れ花あり）</td>
                </tr>
                <tr>
                  <td>寝棺</td>
                  <td>桐六分、布団、仏衣付</td>
                </tr>
                <tr>
                  <td>寝台車</td>
                  <td>基本の10kmまで（吸水シーツ付）</td>
                </tr>
                <tr>
                  <td>火葬お送り</td>
                  <td>火葬場までの基本10kmまで</td>
                </tr>
                <tr>
                  <td>火葬料</td>
                  <td>実費料金</td>
                </tr>
                <tr>
                  <td nowrap="nowrap">収骨容器</td>
                  <td>白瀬戸　7寸(横浜市、川崎市のみ)
他の地域は実費料金となります</td>
                </tr>
                <tr>
                  <td>位牌</td>
                  <td>白木、中上位</td>
                </tr>
                <tr>
                  <td>遺影</td>
                  <td>カラー、四つ切額リボン付、手札付き</td>
                </tr>
                <tr>
                  <td nowrap="nowrap">ドライアイス</td>
                  <td>基本の10kg1回分</td>
                </tr>
                <tr>
                  <td nowrap="nowrap">役所手続き代行</td>
                  <td>火葬手続き代行</td>
                </tr>
                <tr>
                  <td>人件費</td>
                  <td>人件費直葬ランク</td>
                </tr>
                <tr>
                  <td>保管</td>
                  <td>基本1日分</td>
                </tr>
                 <tr>
                  <td>※その他実費</td>
                  <td>休憩料、生花、寺院等お礼、飲食、その他の費用</td>
                </tr>
                <tr>
                  <td colspan="2">
                    <table>
                      <tbody>
                        <tr>
                        <td>地域の風習、しきたりなどにより内容が変わる場合があります。</td>
                      </tr>
                      <tr>
                        <td>バス、ハイヤー、盛物、仕出し料理、返礼品、貸し会場、外注品</td>
                      </tr>
                      <tr>
                        <td>宗教者のお礼、各心づけ等は別途料金となります。</td>
                      </tr>
                    </tbody>
                  </table>
                </td>
              </tr>
            </tbody>
          </table>
        </td>
            <td width="1">&nbsp;</td>
          </tr>
          <!--下部備考-->
        </tbody></table>

        <p id="priceBottom">
        葬儀代金はセット価格にその他費用と実費費用を合計して宗教者へのお礼代金概ね30万をプラスする。<br>
        275,000＋12,750（火葬費＋休憩室：かわさき北部斎苑）＋300,000（お寺）＝587,750円です。（税込）<br/>
        ※その他の費用は含まれていません。
        </p>


       